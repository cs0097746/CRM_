...
def processar_mensagem_media(message_data):
    ...
    Retorna: (texto_msg, tipo_msg, media_url, nome_arquivo, tamanho, duracao, mimetype, base64)
    """
    try:
        ...
        if msg := message_data.get('imageMessage'):
            ...
            logger.info(f"📸 Imagem detectada - URL: {'SIM' if media_url else 'NÃO'}, Base64: {'SIM' if base64_data else 'NÃO'}")
            
            return (texto, 'imagem', media_url, filename, size, None, mimetype, base64_data)
        ...
        # --- ÁUDIO ---
            ...
            logger.info(f"🎵 Áudio detectado - URL: {'SIM' if media_url else 'NÃO'}, Base64: {'SIM' if base64_data else 'NÃO'}")
            
            # Para áudios, precisamos retornar o objeto msg para acesso aos dados de descriptografia
            return (texto, 'audio', msg, filename, size, duration, mimetype, base64_data)
...
def evolution_webhook(request):
    ...
    try:
        ...
        try:
            # Caso IMAGEM - Processar via Base64 (método original que funcionava)
            if tipo_mensagem == "imagem" and base64Text and media_mimetype:
                logger.info(f"� Processando imagem via base64 (método original)")
                base64_clean = base64Text.split(',')[-1] if ',' in base64Text else base64Text
                file_data = base64.b64decode(base64_clean)
                if len(file_data) < 50:  # Reduzir limite - PNG 1x1 tem ~95 bytes
                ...
                if media_mimetype:
                    ...
                    # Se não tem mimetype, usar .jpg como padrão para imagens
                    ext = '.jpg'
                
                # Garantir que o filename tem a extensão correta
                ...
                logger.info(f"🔍 NOME ARQUIVO SALVO: {media_filename} -> CAMINHO: {saved_path}")            # Caso ÁUDIO - Processar com descriptografia WhatsApp + FFmpeg
                ...
                logger.info(f"  - dados_midia: {'PRESENTE' if dados_midia else 'AUSENTE'}")
                
                # Extrair mediaKey do payload para descriptografia (dados_midia é o objeto msg para áudios)
                if dados_midia and isinstance(dados_midia, dict):
                    media_key = dados_midia.get('mediaKey')
                    media_url = dados_midia.get('url')
                    ...
                    if media_url and isinstance(media_url, str) and media_url.startswith('http'):
                        logger.info(f"📡 Baixando áudio da URL: {media_url[:100]}...")
                        
                        # Extrair parâmetros adicionais para descriptografia
                        file_enc_sha256 = dados_midia.get('fileEncSha256')
                ...
                if not file_data and base64Text:
                    try:
                        ...
                        elif dados_midia and dados_midia.get('mediaKey'):
                            # Descriptografar usando mediaKey
                            logger.info("🔐 Iniciando descriptografia com mediaKey...")
                            file_data = WhatsAppDecryption.decrypt_media(encrypted_data, str(dados_midia.get('mediaKey')), 'audio')
                            if file_data:
